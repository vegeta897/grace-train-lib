...
<script lang="ts">
	...
	$: strokeWidthPx = 10 + 10 * strokeWidth
	$: d = stringifyPathCommands(
		(() => {
			const path = lerpPath(defaultPath, taperPath, taper)
			const dipY = 17 + 32 * dip
...
<path {d} stroke-width="{strokeWidthPx}px" fill={outline ? 'none' : undefined} />
